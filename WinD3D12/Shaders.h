...
namespace ver
{
	...
	public:
		...
		static concurrency::task<std::shared_ptr<Shader>>
			MakeAsync(const Graphics& gfx, std::string shader, std::string_view entry);
	private:
	...
	public:
		...
		static std::string GenerateUID(std::string shader, std::string_view entry)noexcept
		{
			return std::format("{}-{}", shader, entry);
		}
