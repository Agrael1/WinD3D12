...
std::vector<std::byte> BasicReaderWriter::ReadData(
...
)
{
    ...
    std::vector<std::byte> fileData(fileInfo.EndOfFile.LowPart);
...
winrt::Windows::Foundation::IAsyncOperation<IBuffer> BasicReaderWriter::ReadDataAsync(
    _In_ winrt::hstring const& filename
)
{
...
)
{
    ...
    co_await FileIO::WriteBytesAsync(file, { fileData.data(), uint32_t(fileData.size()) });
}
